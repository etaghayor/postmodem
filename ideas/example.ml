...
let rec take = function Infinity f -> take (f ());;

print_string "unseen";;

(* Sum Type with two variants *)

type infinity = Halt | GoodInfinity of (unit -> infinity)
...
let unfoldNat = function Zero -> 0 | Succ f -> iterNat f 1
...
type stream = Left of (unit -> stream) | Right of (unit -> stream)
...
let rec traverse = function
  | Left f ->
      print_string "enter_left";
      traverse (f ())
  | Right f ->
      print_string "enter_right";
      traverse (f ())

type tuple = Double of (int * tuple) | Triple of (int * int * tuple)

let rec double =
  Double
    ( 1,
      (print_string "always 42. ";
       double) )
